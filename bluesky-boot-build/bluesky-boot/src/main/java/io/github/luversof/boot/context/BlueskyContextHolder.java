...
public final class BlueskyContextHolder {
        ...
        if (blueskyModuleProperties == null) {
            ...
            if (blueskyPropertiesBeanName
                ...
                return applicationContext.getBean(
                        blueskyPropertiesBeanName, blueskyPropertiesClass);
            }

            String[] beanNamesForType =
                    applicationContext.getBeanNamesForType(blueskyPropertiesClass);
    ...
    private static <T extends BlueskyProperties, U extends BlueskyModuleProperties<T>>
            U getModuleProperties(
                    Class<T> blueskyPropertiesClass, String blueskyPropertiesBeanName) {
        var moduleResolvableType =
                ResolvableType.forClassWithGenerics(
                        BlueskyModuleProperties.class, blueskyPropertiesClass);
        var applicationContext = ApplicationContextUtil.getApplicationContext();

        String[] moduleBeanNamesForType =
                applicationContext.getBeanNamesForType(moduleResolvableType);
        if (moduleBeanNamesForType.length == 0) {
            return null;
        }

        ObjectProvider<U> moduleBeanProvider =
                applicationContext.getBeanProvider(moduleResolvableType);
        ...
        return moduleBeanProvider.stream()
                ...
                .findFirst()
                .orElseThrow(
                        () -> new BlueskyException("NOT EXIST TARGET BLUESKY MODULE PROPERTIES"));
    }
    ...
    public static <T extends BlueskyProperties> BlueskyGroupProperties<T> getGroupProperties(
        ...
        var groupResolvableType =
                ResolvableType.forClassWithGenerics(
                        BlueskyGroupProperties.class, blueskyPropertiesClass);
        var applicationContext = ApplicationContextUtil.getApplicationContext();

        String[] groupBeanNamesForType =
                applicationContext.getBeanNamesForType(groupResolvableType);
