...
public final class ProblemDetailUtil {
  ...
  private static final Logger log = LoggerFactory.getLogger(ProblemDetailUtil.class);
  ...
  private static MessageSourceAccessor getMessageSourceAccessor() {
    if (messageSourceAccessor == null) {
      messageSourceAccessor =
          ApplicationContextUtil.getApplicationContext().getBean(MessageSourceAccessor.class);
    }
  ...
  public static <T extends BlueskyException> ProblemDetail getProblemDetail(T exception) {
    ...
    // api 호출로 발생한 BlueskyException의 경우 exception의 errorMessage또는 errorMessageList에
    // 값이 있음
    // api 호출로 발생한 BlueskyException의 errorMessage의 경우 확장된 errorMessage를 사용
    if (exception.getErrorMessage() != null) {
      problemDetail.setProperty(
          EXCEPTION_PARAMETER, convertBlueskyErrorMessage(exception.getErrorMessage()));
      return problemDetail;
    }

    // api 호출로 발생한 BlueskyException의 errorMessageList는 bindException 결과로
    // BlueskyErrorMessageList 고정임
    if (exception.getErrorMessageList() != null) {
      List<BlueskyErrorMessage> errorMessageList = new ArrayList<>();
      exception.getErrorMessageList().stream()
          .forEach(
              x -> {
   ...
   * @return
   */
  private static <T extends BlueskyException> BlueskyErrorMessage getBlueskyErrorMessage(
      T exception) {
    ...
    var defaultMessageSourceResolvable =
        new DefaultMessageSourceResolvable(
            ...
            exception.getMessage() == null ? exception.getErrorCode() : exception.getMessage());
    ...
    if (StringUtils.hasLength(localizedMessage)
        && !localizedMessage.equals(exception.getErrorCode())
        && !localizedMessage.equals(getMessageSourceAccessor().getMessage("BlueskyException"))) {
  ...
  private static <T extends BlueskyException> String[] getExceptionErrorCodes(T exception) {
    ...
    var errorCodes =
        messageCodesResolver.resolveMessageCodes( // NOSONAR java:S4449
            exception.getClass().getSimpleName(), moduleName, exception.getErrorCode(), null);

    if (exception.getClass().isAssignableFrom(BlueskyException.class)) {
      return errorCodes;
    }

    // BlueskyException을 상속받은 하위 Exception을 사용하면 BlueskyException에 대한 errorCode도
    // 계산하여 가장 긴 순서대로 반환함
    var blueskyErrorCodes =
        messageCodesResolver.resolveMessageCodes( // NOSONAR java:S4449
            BlueskyException.class.getSimpleName(), moduleName, exception.getErrorCode(), null);
    ...
    messageCodeList.sort(
        (v1, v2) ->
            v1.chars().filter(e -> e == '.').count() > v2.chars().filter(e -> e == '.').count()
                ? -1
                : 0);
   ...
   * @return
   */
  private static <T extends ErrorMessage> BlueskyErrorMessage convertBlueskyErrorMessage(
      T errorMessage) {
  ...
  private static void setBlueskyErrorMessage(BlueskyErrorMessage blueskyErrorMessage) {
    ...
    var defaultMessageSourceResolvable =
        new DefaultMessageSourceResolvable(
            ...
            blueskyErrorMessage.getMessage() == null
                ? blueskyErrorMessage.getErrorCode()
                : blueskyErrorMessage.getMessage());
    var localizedMessage = getMessageSourceAccessor().getMessage(defaultMessageSourceResolvable);

    if (StringUtils.hasLength(localizedMessage)
        && !localizedMessage.equals(blueskyErrorMessage.getMessage())) {
  ...
  private static <T extends BindException> List<BlueskyErrorMessage> getBlueskyErrorMessageList(
      T exception) {
    ...
    objectErrorList.forEach(
        objectError -> {
          ...
          log.debug(
              "[MethodArgumentNotValidException error message] code : {}, arguments : {}",
              Arrays.asList(objectError.getCodes()),
              Arrays.asList(objectError.getArguments()));
        });
  ...
  public static <T extends Throwable> ProblemDetail getProblemDetail(
      T exception, HttpStatusCode status) {
    // 로그 처리를 예외 처리한 경우는 제외
    var coreBaseProperties =
        ApplicationContextUtil.getApplicationContext().getBean(CoreBaseProperties.class);
    ...
    var localizedMessage =
        getMessageSourceAccessor()
            .getMessage(exception.getClass().getSimpleName(), exception.getMessage());
