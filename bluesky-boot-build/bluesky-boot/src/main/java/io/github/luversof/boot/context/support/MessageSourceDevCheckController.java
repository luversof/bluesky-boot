...
public class MessageSourceDevCheckController {
  ...
  public MessageSourceDevCheckController(
  ...
  Map<Object, Object> getMessageSources(@RequestParam(required = false) String searchKeyword) {
    ...
    var keyList =
        messageSource.getProperties().keySet().stream()
            .filter(
                key ->
                    searchKeyword == null
  ...
  Map<Object, Object> getMessageSourcesByLocale(
    ...
    keyList.forEach(
        key -> map.put(key, messageSource.getMessage(String.valueOf(key), null, targetLocale)));
    return map;
  }
}
