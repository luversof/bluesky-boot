...
public class DomainModuleNameResolver extends AbstractModuleNameResolver {
  ...
  private final Comparator<Entry<String, DomainProperties>> comparator =
      (Entry<String, DomainProperties> o1, Entry<String, DomainProperties> o2) -> {
  ...
  protected Entry<String, DomainProperties> getModulePropertiesEntry(
      ServerWebExchange exchange, DomainModuleProperties domainModuleProperties) {
    ...
    List<Entry<String, DomainProperties>> moduleEntryList =
        domainModuleProperties.getModules().entrySet().stream()
            .filter(moduleEntry -> checkDomain(exchange, moduleEntry.getValue()))
            .toList();
    ...
    if (moduleEntryList.size() > 1) {
      moduleEntryList =
          moduleEntryList.stream()
              .filter(moduleEntry -> checkDomainWithPath(exchange, moduleEntry.getValue()))
              .toList();
    }
  ...
  private boolean checkDomain(ServerWebExchange exchange, List<URI> uriList) {
    ...
    return uriList.stream()
        .anyMatch(
            uri ->
                uri.getHost().equals(exchange.getRequest().getHeaders().getHost().getHostName()));
  }

  private boolean checkDomainWithPath(
      ServerWebExchange exchange, DomainProperties domainProperties) {
    return domainProperties != null
            ...
            uri ->
                uri.getHost().equals(exchange.getRequest().getHeaders().getHost().getHostName())
                    && (exchange.getRequest().getURI().getPath().startsWith(uri.getPath())
                        || (exchange.getRequest().getURI().getPath().length() + 1
                                == uri.getPath().length()
                            && (exchange.getRequest().getURI().getPath() + "/")
                                .equals(
                                    uri.getPath())) // request sevletPath가 설정된 path보다 length 가 1 짧은
                    // 경우 / 를 추가하여 동일 여부 체크
                    ));
  }
}
