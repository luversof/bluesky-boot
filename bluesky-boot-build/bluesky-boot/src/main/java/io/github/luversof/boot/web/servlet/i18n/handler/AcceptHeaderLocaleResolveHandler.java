...
public class AcceptHeaderLocaleResolveHandler extends AbstractLocaleResolveHandler {
  ...
  /**
   * When creating an AcceptHeaderLocaleResolveHandler, localePropertiesBeanName and
   * localeResolveHandlerPropertiesBeanName are required.
   *
   ...
   */
  public AcceptHeaderLocaleResolveHandler(
  ...
  /**
   * request accept header에서 Locale을 구함 현재는 단일 Locale 반환으로 처리하고 있는데 만약 복수 처리가 필요하다면 관련하여
   * request.getLocales() 를 사용하는 처리로 개편 필요
   *
   * @param request
   * @return
   */
  private Locale getRequestLocale(HttpServletRequest request) {
    ...
    return request.getLocale(); // accept language header의 parse 된 첫번째 locale 반환. header가 없으면
    // Locale.getDefault() 반환
  }
