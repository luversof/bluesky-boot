 ...
 * ContextRefresher 빈과 중복 선언되면 안되기 때문에 내부 변수로 새로 생성하여 사용
 */
public class BlueskyPropertiesRefresher
  ...
  /**
   * refresh는 dependency 참조를 기준으로 수행하지 않아 참조된 bean이 먼저 생성되는 보장이 없음 우선 호출해야 할 대상 목록을 지정하여 먼저 refresh
   * 처리
   */
  ...
  public BlueskyPropertiesRefresher(
    ...
    this.configDataContextRefresher =
        new ConfigDataContextRefresher(context, scope, properties) {
        ...
        .forEach(
            (key, value) ->
                BeanUtils.copyProperties(
                    ...
                    context.getBean(key, BlueskyRefreshProperties.class)));
  ...
  /**
   * blueskyProperties 갱신 처리 우선 갱신 해야 하는 3개를 먼저 갱신 처리한 후 나머지는 순차 갱신 처리한다. 만약 더 잘 구성하고 싶다면 bean의
   * dependency를 참조해서 순서를 구성해야 하지만 참조 빈이 갱신되지 않으므로 이는 추후 개선이 필요하다.
   *
   * @param keys
   */
