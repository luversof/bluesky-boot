...
/**
 * Abstract classes of CookieLocaleResolveHandler and
 * AcceptHeaderLocaleResolveHandler
 */
public abstract class AbstractLocaleResolveHandler implements LocaleResolveHandler {
	 ...
	 * In the case of LocaleProperties, multiple beans can be registered and used,
	 * so search based on localePropertiesBeanName
	 * 
	 ...
	 * In the case of LocaleResolveHandlerProperties, multiple beans can be
	 * registered and used, so search based on
	...
	protected LocaleResolveHandlerProperties getLocaleResolveHandlerProperties() {
		return BlueskyContextHolder.getProperties(LocaleResolveHandlerProperties.class,
				localeResolveHandlerPropertiesBeanName);
	}
	 ...
	 * set localeContext
	 * 
	 * @param localeResolveInfo          An object containing information related to
	 *                                   locale resolve processing.
	 * @param localeResolveInfoContainer Passed in case of referencing the preceding
	 *                                   localeResolveInfo
	 */
	protected void setResolveLocale(LocaleResolveInfo localeResolveInfo,
		...
		if (PreLocaleResolveInfoCondition.USE_WHEN_NOT_RESOLVED.equals(preLocaleResolveInfoCondition)
				|| PreLocaleResolveInfoCondition.USE_LANGUAGE_WHEN_NOT_RESOLVED.equals(preLocaleResolveInfoCondition)) {
			resolveLocale = getResolveLocaleByPreResolveInfo(localeResolveInfoContainer);
		}
	 ...
	 * 
	 * @param localeResolveInfoContainer Container containing locale information to
	 *                                   use
	 * @param localeResolveInfo          An object containing information related to
	 *                                   locale resolve processing.
	 */
	protected void addLocaleResolveInfo(LocaleResolveInfoContainer localeResolveInfoContainer,
	 ...
	 *                                   locale resolve processing.
	 */
	protected void setRepresenatativeSupplier(LocaleResolveInfoContainer localeResolveInfoContainer,
			LocaleResolveInfo localeResolveInfo) {
		var localeResolveHandlerProperties = getLocaleResolveHandlerProperties();
		...
		switch (setRepresentativeCondition == null ? SetRepresentativeCondition.NONE : setRepresentativeCondition) {
			...
			case OVERWRITE_IF_NOT_EXISTS:
				if (localeResolveInfoContainer.getRepresentativeSupplier() == null
						|| localeResolveInfoContainer.getRepresentativeSupplier().get().getResolveLocale() == null) {
					localeResolveInfoContainer.setRepresentativeSupplier(() -> localeResolveInfo);
				}
				break;
			default:
	...
	/**
	 * Calculate resolveLocale based on the locale calculated in the preceding
	 * handler
	 * 
	 * @param localeResolveInfoContainer Container containing locale information to
	 *                                   use
