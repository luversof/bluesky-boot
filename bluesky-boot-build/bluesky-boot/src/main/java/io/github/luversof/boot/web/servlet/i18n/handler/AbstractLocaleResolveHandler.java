...
public abstract class AbstractLocaleResolveHandler implements LocaleResolveHandler {
  ...
  AbstractLocaleResolveHandler(
      String localePropertiesBeanName, String localeResolveHandlerPropertiesBeanName) {
  ...
  public String getLocalePropertiesBeanName() {
    return localePropertiesBeanName;
  }

  public String getLocaleResolveHandlerPropertiesBeanName() {
    return localeResolveHandlerPropertiesBeanName;
  }
  ...
  /**
   * Search for LocaleProperties based on Context In the case of LocaleProperties, multiple beans
   * can be registered and used, so search based on localePropertiesBeanName
   *
  ...
  /**
   * Search for LocaleResolveHandlerProperties based on Context In the case of
   * LocaleResolveHandlerProperties, multiple beans can be registered and used, so search based on
   * localeResolveHandlerPropertiesBeanName
   *
   * @return LocaleResolveHandlerProperties
   */
  protected LocaleResolveHandlerProperties getLocaleResolveHandlerProperties() {
    return BlueskyContextHolder.getProperties(
        LocaleResolveHandlerProperties.class, localeResolveHandlerPropertiesBeanName);
  }
   ...
   */
  protected void setResolveLocale(
      LocaleResolveInfo localeResolveInfo, LocaleResolveInfoContainer localeResolveInfoContainer) {
    ...
    if (PreLocaleResolveInfoCondition.USE_FIRST.equals(preLocaleResolveInfoCondition)
        || PreLocaleResolveInfoCondition.USE_LANGUAGE_FIRST.equals(preLocaleResolveInfoCondition)) {
    ...
    if (PreLocaleResolveInfoCondition.USE_WHEN_NOT_RESOLVED.equals(preLocaleResolveInfoCondition)
        || PreLocaleResolveInfoCondition.USE_LANGUAGE_WHEN_NOT_RESOLVED.equals(
   ...
   * @param localeResolveInfo An object containing information related to locale resolve processing.
   */
  protected void addLocaleResolveInfo(
      LocaleResolveInfoContainer localeResolveInfoContainer, LocaleResolveInfo localeResolveInfo) {
   ...
   * @param localeResolveInfo An object containing information related to locale resolve processing.
   */
  protected void setRepresenatativeSupplier(
      LocaleResolveInfoContainer localeResolveInfoContainer, LocaleResolveInfo localeResolveInfo) {
    var localeResolveHandlerProperties = getLocaleResolveHandlerProperties();
    var setRepresentativeCondition = localeResolveHandlerProperties.getSetRepresentativeCondition();

    switch (setRepresentativeCondition == null
        ? SetRepresentativeCondition.NONE
        : setRepresentativeCondition) {
      case NONE:
        break;
      case OVERWRITE:
      ...
      case OVERWRITE_IF_NOT_EXISTS:
        if (localeResolveInfoContainer.getRepresentativeSupplier() == null
            || localeResolveInfoContainer.getRepresentativeSupplier().get().getResolveLocale()
                == null) {
          localeResolveInfoContainer.setRepresentativeSupplier(() -> localeResolveInfo);
        }
        break;
   ...
   * @return resolved locale
   */
  protected Locale getResolveLocaleByPreResolveInfo(
      LocaleResolveInfoContainer localeResolveInfoContainer) {
    var resolveList = localeResolveInfoContainer.getResolveList();
    var preResolveLocale =
        CollectionUtils.isEmpty(resolveList) ? null : resolveList.getLast().getResolveLocale();
    ...
    if (preResolveLocale != null) {
      ...
      return getResolveLocale(
          preResolveLocale, preLocaleResolveInfoCondition.isCheckLanguageMatchOnly());
    }
    return null;
  }
}
