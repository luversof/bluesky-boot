...
public class DomainModuleNameResolver extends AbstractModuleNameResolver {
    ...
    if (moduleEntryList.size() > 1) {
      moduleEntryList =
          moduleEntryList.stream()
              .filter(moduleEntry -> checkDomainWithPath(request, moduleEntry.getValue()))
              .toList();
    }
  ...
  private boolean checkDomainWithPath(
      HttpServletRequest request, DomainProperties domainProperties) {
    if (request == null) {
      return false;
    }
  ...
  private boolean checkDomainWithPath(HttpServletRequest request, List<URI> uriList) {
    ...
    return uriList.stream()
        .anyMatch(
            uri ->
                uri.getHost().equals(request.getServerName())
                    && (request.getServletPath().startsWith(uri.getPath())
                        || (request.getServletPath().length() + 1 == uri.getPath().length()
