...
public final class ExceptionUtil {
	...
	private static List<ErrorViewResolver> getErrorViewResolverList() {
		if (errorViewResolverList == null) {
			errorViewResolverList = ApplicationContextUtil.getApplicationContext()
					.getBeanProvider(ErrorViewResolver.class).orderedStream().toList();
		}
		return errorViewResolverList;
	}

	/**
	 * If the response is json, return a ProblemDetail object, otherwise return a
	 * ModelAndView object using ErrorViewResolver.
	 * 
	 ...
	 *         situation.
	 */
	public static Object handleException(ProblemDetail problemDetail, Object handler,
		...
		if (ExceptionUtil.isJsonResponse(handler, nativeWebRequest)) {
			...
			for (ErrorViewResolver resolver : getErrorViewResolverList()) {
				...
				ModelAndView modelAndView = resolver.resolveErrorView(httpServletRequest,
						HttpStatus.valueOf(problemDetail.getStatus()), problemDetail.getProperties());
	...
	private static boolean isJsonResponse(Object handler, NativeWebRequest request) {
		try {
			var contentNegotiationManager = ApplicationContextUtil.getApplicationContext()
			...
			if (methodAnnotation != null
					&& Arrays.asList(methodAnnotation.produces()).contains(MediaType.APPLICATION_JSON_VALUE)) {
				return true;
			}
