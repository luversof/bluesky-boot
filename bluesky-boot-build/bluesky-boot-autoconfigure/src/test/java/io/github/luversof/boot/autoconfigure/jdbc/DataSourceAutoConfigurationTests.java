...
class DataSourceAutoConfigurationTests {
    ...
    void testWithConnectionInfo() {
        this.contextRunner
                ...
                .withPropertyValues(
                        "bluesky-boot.core.modules.test.core-module-info=T(io.github.luversof.boot.autoconfigure.core.constant.TestCoreModuleInfo).TEST")
                .withUserConfiguration(CORE_USER_CONFIGURATION)
                .withUserConfiguration(ConnectionInfoAutoConfiguration.class)
                .run(
                        context -> {
                            DataSource dataSource =
                                    context.getBean("routingDataSource", DataSource.class);
                            ...
                            if (dataSource
                                ...
                                if (targetDataSource
                                        instanceof RoutingDataSource routingDataSource) {
                                    var resolvedDataSources =
                                            routingDataSource.getResolvedDataSources();
                                    var testDataSource =
                                            DataSourceBuilder.create(null)
