...
class RedisAutoConfigurationTests {
  ...
  void testDefaultRedisConfiguration() {
    this.contextRunner.run(
        context -> {
          ...
          assertThat(context.getBean(RedisConnectionFactory.class))
  ...
  void testRedisUrlConfiguration() {
    this.contextRunner
        .withPropertyValues(
            "spring.redis.host:foo", "spring.redis.url:redis://user:password@example:33") // NOSONAR
        // secrets:S6739
        .run(
            context -> {
              LettuceConnectionFactory cf = context.getBean(LettuceConnectionFactory.class);
  ...
  void testOverrideUrlRedisConfiguration() {
    this.contextRunner
        .withPropertyValues(
            ...
            "spring.redis.url:rediss://user:password@example:33") // NOSONAR
        // secrets:S6739
        .run(
            context -> {
              LettuceConnectionFactory cf = context.getBean(LettuceConnectionFactory.class);
  ...
  void testPasswordInUrlWithColon() {
    this.contextRunner
        .withPropertyValues("spring.redis.url:redis://user:password@example:33")
        .run(
            context -> { // NOSONAR
              // secrets:S6739
              LettuceConnectionFactory cf = context.getBean(LettuceConnectionFactory.class);
    ...
    this.contextRunner
        ...
        .run(
            context -> {
              ...
              assertThat(poolConfig.getDurationBetweenEvictionRuns())
  ...
  void testRedisConfigurationWithClientName() {
    this.contextRunner
        .withPropertyValues("spring.redis.host:foo", "spring.redis.client-name:spring-boot")
        .run(
            context -> {
              LettuceConnectionFactory cf = context.getBean(LettuceConnectionFactory.class);
              assertThat(cf.getHostName()).isEqualTo("foo");
  ...
  void connectionFactoryWithJedisClientType() {
    this.contextRunner
        .withPropertyValues("spring.redis.client-type:jedis")
        .run(
            context -> {
              assertThat(context).hasSingleBean(RedisConnectionFactory.class);
              assertThat(context.getBean(RedisConnectionFactory.class))
  ...
  void connectionFactoryWithLettuceClientType() {
    this.contextRunner
        .withPropertyValues("spring.redis.client-type:lettuce")
        .run(
            context -> {
              assertThat(context).hasSingleBean(RedisConnectionFactory.class);
              assertThat(context.getBean(RedisConnectionFactory.class))
  ...
  void testRedisConfigurationWithSentinel() {
    ...
    this.contextRunner
        ...
        .run(
            context ->
                assertThat(context.getBean(LettuceConnectionFactory.class).isRedisSentinelAware())
                    .isTrue());
  }
        ...
        .run(
            context -> {
              ...
              RedisSentinelConfiguration sentinelConfiguration =
                  connectionFactory.getSentinelConfiguration();
              assertThat(sentinelConfiguration.getSentinelPassword().isPresent()).isFalse();
              Set<RedisNode> sentinels =
                  connectionFactory.getSentinelConfiguration().getSentinels();
    ...
    this.contextRunner
        ...
        .run(
            context -> {
              ...
              RedisSentinelConfiguration sentinelConfiguration =
                  connectionFactory.getSentinelConfiguration();
              assertThat(new String(sentinelConfiguration.getSentinelPassword().get()))
                  .isEqualTo("secret");
        ...
        .run(
            context -> {
              RedisClusterConfiguration clusterConfiguration =
                  context.getBean(LettuceConnectionFactory.class).getClusterConfiguration();
    ...
    this.contextRunner
        ...
        .run(
            assertClientOptions(
                ClusterClientOptions.class,
                options ->
                    assertThat(options.getTopologyRefreshOptions().getAdaptiveRefreshTriggers())
  ...
  void testRedisConfigurationWithClusterRefreshPeriodHasNoEffectWithNonClusteredConfiguration() {
    this.contextRunner
        .withPropertyValues("spring.redis.cluster.refresh.period=30s")
        .run(
            assertClientOptions(
                ClientOptions.class,
    ...
    this.contextRunner
        ...
        .run(
            assertClientOptions(
                ClusterClientOptions.class,
                options ->
                    assertThat(options.getTopologyRefreshOptions().useDynamicRefreshSources())
                        .isTrue()));
  }
    ...
    this.contextRunner
        ...
        .run(
            context -> {
              ...
              HashOperations<String, String, Object> redisOperations =
                  context.getBean("redisTemplate", HashOperations.class);
  ...
  void testBlueskyStringRedisTemplate() {
    this.contextRunner
        ...
        context -> {
          @SuppressWarnings("unchecked")
          HashOperations<String, String, Integer> redisOperations =
              context.getBean("stringRedisTemplate", HashOperations.class);
  ...
  private <T extends ClientOptions>
      ContextConsumer<AssertableApplicationContext> assertClientOptions(
          Class<T> expectedType, Consumer<T> options) {
    return context -> {
      LettuceClientConfiguration clientConfiguration =
          context.getBean(LettuceConnectionFactory.class).getClientConfiguration();
  ...
  private LettucePoolingClientConfiguration getPoolingClientConfiguration(
    ...
    return (LettucePoolingClientConfiguration)
        ReflectionTestUtils.getField(factory, "clientConfiguration");
  }
